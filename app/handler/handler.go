...
import (
	...
	"github.com/bianjieai/ddc-sdk-go/ddc-sdk-platform-go/pkg/contracts/contract"
...
// GetDDC721 获取DDC721合约实体
func GetDDCMyNft() *contract.Contract {
	cTmp := config.Info
	fmt.Println(cTmp)
	ddc721, err := contract.NewContract(config.Info.DdcMyNFtAddress(), config.Info.Conn())
	if err != nil {
		log.Error.Printf("failed to get contract DDC721:%v", err)
	}
	return ddc721
